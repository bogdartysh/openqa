 ...
 * 
 * <p>
 * All print methods are thread-save to avoid overlapping outputs from different
 * threads.
 * </p>
 * 
...
public class MsgPrinter {
	 ...
	 * 
	 * @param enable
	 *            <code>true</code> to enable error messages, <code>false</code>
	 *            to disable them
	 */
	...
	public static synchronized void printStatusMsgTimestamp(String status) {
		if (statusMsgs)
			printStatusMsg(status + " (" + getTimestamp() + ")");
	}
	 ...
	 * 
	 * @param qis
	 *            <code>QuestionInterpretation</code> array
	 */
	 ...
	 * 
	 * @param filter
	 *            the filter that has just started its work
	 * @param resCount
	 *            the number of results passed to the filter
	 */
	 ...
	 * 
	 * @param filter
	 *            the filter that has just finished its work
	 * @param resCount
	...
	public static synchronized void printErrorMsgTimestamp(String error) {
		if (errorMsgs)
			printErrorMsg(error + " (" + getTimestamp() + ")");
	}
	 ...
	 * 
	 * @param e
	 *            an <code>Exception</code> that has been thrown
	 */
	 ...
	 */
	public static synchronized void printAnswers(Collection<Result> results) {
		printMessage("\nAnswer:");
		int i = 0;
		for (Result result : results) {
			printMessage("[" + (i + 1) + "]\t" + result.getAnswer());
			printMessage("\tScore: " + result.getScore());
			if (result.getDocID() != null)
				printMessage("\tDocument: " + result.getDocID());
			i++;
		}
	}
	 ...
	 */
	private static SimpleDateFormat timestampFormatter = new SimpleDateFormat(
			"yyyy-MM-dd HH:mm:ss");
	 ...
	 * 
	 * @param logFile
	 *            the path and name of the filte to write log entries to (in
	 *            addition to System.out)
	 */
